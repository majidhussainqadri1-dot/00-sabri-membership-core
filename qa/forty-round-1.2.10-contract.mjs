...
const checks = [
  ['current runtime version', plugin.includes("define( 'SMC_VERSION', '1.2.37' );")],
  ['current package version', packageJson.version === '1.2.37'],
  ...
  ['restore evidence server validation', completion.includes("strlen( $reference ) >= 8") && completion.includes("smc_restore_proof_v1")],
  ['no master-secret-derived key identifier', !completion.includes("hash( 'sha256', (string) SMC_MASTER_KEY )")],
  ['explicit non-secret key ID', security.includes('SMC_MASTER_KEY_ID') && security.includes("smc_key_id_missing")],
...
console.log(`${checks.length}/${checks.length} retained forty-round corrective assertions passed for current runtime 1.2.37; historical 1.2.10 evidence remains immutable.`);
