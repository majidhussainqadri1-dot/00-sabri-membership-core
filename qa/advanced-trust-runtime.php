...
class SMC_Contracts {
  public static function assertions($id){ return [
    'identity_documents_current'=>true,'phone_verified'=>true,'email_verified'=>true,'guardian_verified'=>true,
...
$a=SMC_Advanced_Trust_2026::authentication_assurance(7); t('spoofed File02 elevation rejected', $a['level']===2 && empty($a['passkey_asserted']));
...
$a=SMC_Advanced_Trust_2026::authentication_assurance(7); t('fresh File02 passkey claim accepted', $a['level']===3 && !empty($a['passkey_asserted']) && !empty($a['hardware_backed']));
...
t('revocation epoch and SLA emitted', is_array($rev) && $rev['sla_seconds']===60 && in_array('smc_trust_revocation_invalidated',$actions,true));
...
$chg=SMC_Advanced_Trust_2026::mark_critical_identity_change(7,'date_of_birth',1,'correction'); $m=SMC_Advanced_Trust_2026::minimal_assertions(7,'file17'); t('critical DOB change invalidates age assertion', $chg===true && empty($m['age_requirement_met']));
...
t('assurance profile reaches verified level', $profile['identity_assurance_level']>=3 && $profile['authentication_assurance_level']>=2);
