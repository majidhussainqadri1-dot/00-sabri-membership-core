...
const checks = [
  ['runtime 1.2.37', plugin.includes("define( 'SMC_VERSION', '1.2.37' );")],
  ['contract 1.2.2', plugin.includes("define( 'SMC_CONTRACT_VERSION', '1.2.2' );")],
  ...
  ['File 00 MFA retired', contracts.includes("'mfa_required'           => false") && contracts.includes("'mfa_owner'              => 'none'")],
...
console.log(`${checks.length}/${checks.length} three-plan completion assertions passed for current runtime 1.2.37 while historical traceability remains immutable.`);
