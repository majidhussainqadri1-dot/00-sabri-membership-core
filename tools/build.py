...
def source_files() -> list[Path]:
    return sorted((path for path in PLUGIN.rglob("*") if path.is_file() and not path.is_symlink() and path.name != "MANIFEST.sha256" and "__pycache__" not in path.parts), key=lambda path: path.relative_to(PLUGIN).as_posix())
