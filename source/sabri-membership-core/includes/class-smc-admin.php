...
final class SMC_Admin {
	...
	private static function approve( $user_id, $request, $version, $reason ) {
		...
		$ok3 = $wpdb->update( $wpdb->prefix . 'smc_identity_records', array( 'name_match_status' => 'matched', 'name_match_note' => $reason, 'verified_at' => $now, 'verified_by' => get_current_user_id(), 'updated_at' => $now ), array( 'user_id' => $user_id ) );
